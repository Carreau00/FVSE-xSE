...
namespace FVSETranslator
{
	void LoadTranslations(BSScaleformTranslator * translator)
	{
		...
		// grab the _en files first.  That way if something in the translation file isn't filled, we won't get $$Bill Yo!
		ParseTranslation(translator, "FalloutVore", "en");

		if (strcmp(setting->data.s, "en") != 0) {
			ParseTranslation(translator, "FalloutVore", setting->data.s);
		}
	}
