...
public:
    ...
    static void UpdateSingle(std::shared_ptr<RVAData> rvaData, UInt32 runtimeVersion = 0) {
        if (SHOW_ADDR != 1 && rvaData->addr.count(runtimeVersion) > 0) {
			...
			_MESSAGE("---");
			_MESSAGE("addr.count(runtimeVersion) %d", rvaData->addr.count(runtimeVersion));
...
private:
    ...
    void init(AddressMap addr, const char* sig, int offset, int indirectOffset = 0, int instructionLength = 0, const char* name = "no name") {
